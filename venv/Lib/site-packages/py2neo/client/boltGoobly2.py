...
class Bolt3(Bolt2):
    ...
    def ready(self, the_object, the_class):
        # print(the_object.__dict__.keys())
        # print(type(f"self.{the_object.__dict__.keys()}" ))
        # print(type(the_object._transaction._items)) == deque([])

        # print(f"the object is {the_object}")
        from pprint import pprint
        # # pprint(the_object.__dict__.keys())
        # pprint(vars(the_object))
        # u = [attr for attr in dir(the_object) ]#if not callable(getattr(obj(),attr)) and not attr.startswith("__")
        # pprint(u)
        print(f"printing: {the_class}")
        # print("all dir ^")
        # u = [attr for attr in dir(the_object) if not callable(getattr(the_object,attr)) and not attr.startswith("__") ]
        # pprint(u)
        # print("doesn't start _ ^")
        u = [attr for attr in dir(the_object) if not callable(getattr(the_object,attr)) ]
        pprint(u)        
        pprint(getattr(the_object,'_metadata'))
        ...
        # import sys
        # print("this?")
        # print(sys._getframe(1).f_code.co_name) #_run
        # pprint(stack[2][0].f_code.co_name)#'auto_run'
        # pprint(stack[1][0].f_code.co_name)#_run
        # pprint(stack[3][0].f_code.co_name)#'auto_run'
        # # pprint(stack[1][1].f_code.co_name)
        # pprint(stack[1][0])
        # u = [attr for attr in dir(stack[1][0]) if not callable(getattr(stack[1][0],attr)) ]
        # pprint(u)
        # # for attr in dir(stack[1][0]):
        # #     if not callable(getattr(stack[1][0],attr)):
        # #         attr
        # pprint(stack[1][0].f_back)
        # pprint(stack[1][0].f_code)
        # pprint(stack[1][0].f_globals)
        # pprint(stack[1][0].f_lasti)
        # pprint(stack[1][0].f_locals)

        # print("f_back")
        # u = [attr for attr in dir(stack[1][0].f_back) if not callable(getattr(stack[1][0].f_back,attr)) ]
        # pprint(u)
        # print("f_code")
        # u = [attr for attr in dir(stack[1][0].f_code) if not callable(getattr(stack[1][0].f_code,attr)) ]
        # pprint(u)
        # print("f_globals")
        # u = [attr for attr in dir(stack[1][0].f_globals) if not callable(getattr(stack[1][0].f_globals,attr)) ]
        # pprint(u)
        # print("f_lasti")
        # u = [attr for attr in dir(stack[1][0].f_lasti) if not callable(getattr(stack[1][0].f_lasti,attr)) ]
        # pprint(u)
        # print("f_locals")
        # u = [attr for attr in dir(stack[1][0].f_locals) if not callable(getattr(stack[1][0].f_locals,attr)) ]
        # pprint(u)
        
    def ready2(self, the_object):
        from pprint import pprint
        import inspect
        # print(inspect.stack()[1].function)
        # print(inspect.stack()[1].co_name)
        ...
        pprint(stack[1][0].f_code.co_firstlineno)

    def _run(self, graph_name, cypher, parameters, extra=None, final=False):
        import logging, sys, inspect
        from pprint import pprint
        logging.warning('%s before you %s', pprint(self._transaction._items), cypher)
        # self.ready(self, sys._getframe(0).f_locals['self'].__class__)
        self.ready2(self)
        ...
        print("the ccccccc")
        pprint(the_class)
        self.ready(self, inspect.currentframe().f_back.f_locals["self"].__class__ )
